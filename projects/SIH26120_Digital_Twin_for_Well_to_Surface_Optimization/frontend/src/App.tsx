    ...
    try {
      ...
      const res = await fetch(`${apiUrl}/api/v1/analyze`, {
        ...
        body: JSON.stringify({
          ...
          metric_value: metric,
          location_label: 'Oil India Limited'
        })
      });
      ...
      setResult({
        ...
        action_taken: metric > 75 ? 'Automated alert webhook dispatched to Oil India Limited SPOC' : 'Telemetry logged in Supabase database'
      });
    } finally {
      setLoading(false);
    }
    ...
    <div className="min-h-screen bg-slate-950 text-slate-100 font-sans p-4 sm:p-6 lg:p-8">
      <div className="max-w-7xl mx-auto space-y-6">
        ...
        <header className="bg-slate-900/90 border border-slate-800 p-6 rounded-3xl flex flex-col md:flex-row items-start md:items-center justify-between gap-4 shadow-xl">
          <div>
            <div className="text-xs font-mono font-bold text-emerald-400 mb-1">SIH26120 • Smart Automation</div>
            <h1 className="text-xl font-bold text-white">Digital Twin for Well-to-Surface Optimization of Cyclic Steam Stimulation (CSS) and Sucker Rod Pump (SRP) Operations for Heavy Oil Wells of Baghewala Field.</h1>
            <p className="text-xs text-slate-400 mt-1">Oil India Limited • Smart India Hackathon 2026</p>
          </div>
