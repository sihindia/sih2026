    ...
    try {
      ...
      const res = await fetch(`${apiUrl}/api/v1/analyze`, {
        ...
        body: JSON.stringify({
          ...
          metric_value: metric,
          location_label: 'Ministry of Earth Sciences (MoES)'
        })
      });
      ...
      setResult({
        ...
        action_taken: metric > 75 ? 'Automated alert webhook dispatched to Ministry of Earth Sciences (MoES) SPOC' : 'Telemetry logged in Supabase database'
      });
    } finally {
      setLoading(false);
    }
    ...
    <div className="min-h-screen bg-slate-950 text-slate-100 font-sans p-4 sm:p-6 lg:p-8">
      <div className="max-w-7xl mx-auto space-y-6">
        ...
        <header className="bg-slate-900/90 border border-slate-800 p-6 rounded-3xl flex flex-col md:flex-row items-start md:items-center justify-between gap-4 shadow-xl">
          <div>
            <div className="text-xs font-mono font-bold text-emerald-400 mb-1">SIH26075 • Smart Education</div>
            <h1 className="text-xl font-bold text-white">Participants are invited to design and develop **CAPACITY CONNECT A Digital Capacity Building and Learning Management Portal** to support organizational training, competency development, and knowledge sharing through a centralized web-based platform.</h1>
            <p className="text-xs text-slate-400 mt-1">Ministry of Earth Sciences (MoES) • Smart India Hackathon 2026</p>
          </div>
