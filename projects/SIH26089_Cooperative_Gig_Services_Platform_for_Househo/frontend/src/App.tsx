    ...
    try {
      ...
      const res = await fetch(`${apiUrl}/api/v1/analyze`, {
        ...
        body: JSON.stringify({
          ...
          metric_value: metric,
          location_label: 'Ministry of Cooperation'
        })
      });
      ...
      setResult({
        ...
        action_taken: metric > 75 ? 'Automated alert webhook dispatched to Ministry of Cooperation SPOC' : 'Telemetry logged in Supabase database'
      });
    } finally {
      setLoading(false);
    }
    ...
    <div className="min-h-screen bg-slate-950 text-slate-100 font-sans p-4 sm:p-6 lg:p-8">
      <div className="max-w-7xl mx-auto space-y-6">
        ...
        <header className="bg-slate-900/90 border border-slate-800 p-6 rounded-3xl flex flex-col md:flex-row items-start md:items-center justify-between gap-4 shadow-xl">
          <div>
            <div className="text-xs font-mono font-bold text-emerald-400 mb-1">SIH26089 • Agriculture, FoodTech & Rural Development</div>
            <h1 className="text-xl font-bold text-white">Cooperative Gig Services Platform for Household & Community Services</h1>
            <p className="text-xs text-slate-400 mt-1">Ministry of Cooperation • Smart India Hackathon 2026</p>
          </div>
